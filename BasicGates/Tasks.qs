...
namespace Quantum.Kata.BasicGates {
    ...
    operation AmplitudeChange (alpha : Double, q : Qubit) : Unit is Adj+Ctl {
		//Note that doubling is needed since rotation of a bloch sphere is half that of a geometric sphere
        Ry(2.0 * alpha, q);
    }
    ...
    operation TwoQubitGate3 (qs : Qubit[]) : Unit is Adj {
		...
		//Solution using only one gate:
		// SWAP(qs[0], qs[1]);

		//Solution using multiple gates:
		// start		CNOT 1		CNOT 2		CNOT 3
		// 00			00			00			00
		// 01			01			11			10
		// 10			11			01			01
		// 11			10			10			11
